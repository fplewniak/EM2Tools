...
class GetSet(argparse.Action):
    """
An argparse custom action to return a set from an argument containing a list of elements and/or file names. Files are
supposed to contain one element of the list per line. There can be more than one file and the argument may take a
combination of elements and files. In all cases, the returned set will contain all the specified elements keeping only
one copy of each element. If you do not want to remove redundancy, then use GetList action instead.
    """
    def __init__(self, option_strings, dest, nargs='+', **kwargs):
        if nargs != '+':
            raise ValueError('GetList get_list_action requires nargs to be set to "+"')
